...
PRODUCT_CATALOG = [
    ...
    # Extra Snacks (5) to reach 135
    ("Sunfeast Bounce",              "Snacks",         "ITC",        "82g",    28,  40,  12),
    ("Jim Jam Biscuit",              "Snacks",         "Britannia",  "150g",   22,  32,  12),
    ("Hide & Seek Biscuit",          "Snacks",         "Parle",      "120g",   32,  45,  12),
    ("Nutrichoice Biscuit",          "Snacks",         "Britannia",  "100g",   35,  52,  12),
    ("Tiger Biscuit",                "Snacks",         "Britannia",  "100g",   18,  26,  12),
    # Extra Household (5) to reach 135
    ("Harpic Bathroom Cleaner",      "Household",      "Reckitt",    "500ml",  78,  120, 18),
    ("Lizol Disinfectant Spray",     "Household",      "Reckitt",    "500ml",  68,  105, 12),
    ("Mortein Insect Spray",         "Household",      "Reckitt",    "300ml",  142, 215, 12),
    ("Colin Multipurpose Cleaner",   "Household",      "Reckitt",    "500ml",  62,  95,  12),
    ("Dettol Antiseptic Liquid",     "Household",      "Reckitt",    "250ml",  75,  115, 12),
]
...
def gen_sales_recv_pay(customer_df: pd.DataFrame, item_df: pd.DataFrame):
    ...
    for _, cust in customer_df.iterrows():
        ...
        if arch == "inactive":
            ...
            for _ in range(n_inv):
                ...
                # O(n_lines) patch — track start index before appending lines
                for r2 in sales_rows[start_idx:]:
    ...
    # Scale toward target by sampling at invoice level
    if not s.empty:
        all_vnos = s["voucher_no"].unique()
        # Calculate avg lines per invoice
        avg_lines = len(s) / len(all_vnos)
        target_invoices = int(TARGET_SALES_LINES / max(avg_lines, 1))
        if len(all_vnos) > target_invoices:
            kept_v = set(rng.choice(all_vnos, size=target_invoices, replace=False))
        ...
        print(f"  After scaling: {len(s)} lines / {s['voucher_no'].nunique()} invoices")
...
def gen_purchases(sales_df: pd.DataFrame, item_df: pd.DataFrame) -> pd.DataFrame:
    prod_map = {r["product_id"]: r for r in item_df.to_dict("records")}
...
def gen_stock(item_df, sales_df, purch_df) -> pd.DataFrame:
    # Pre-aggregate totals by product_id
    sales_qty = {} if sales_df.empty else sales_df.groupby("product_id")["qty"].sum().to_dict()
    purch_qty = {} if purch_df.empty else purch_df.groupby("product_id")["qty"].sum().to_dict()
    rows = []
    for prod in item_df.to_dict("records"):
        pid = prod["product_id"]
        sq  = int(sales_qty.get(pid, 0))
        pq  = int(purch_qty.get(pid, 0))
        for gi, gd in enumerate(GODOWNS):
            gr = np.random.default_rng(SEED + hash(pid+gd)%100000)
            gf = [0.6, 0.25, 0.15][gi]
            gs = int(sq*gf); gp = int(pq*gf)
            op = int(gr.integers(5, max(6, int(prod["opening_stock"]*gf)+20)))
            cl = max(0, op+gp-gs)
...
def build_proc_customers(customer_df, recv_df, sales_df) -> pd.DataFrame:
    ...
    # Build dict lookups for O(1) access
    ra_dict = ra.set_index("customer_id")[["outstanding_amount","lp"]].to_dict("index") if not ra.empty else {}
    sa_dict = sa.set_index("customer_id")[["last_order_date","total_business_ytd"]].to_dict("index") if not sa.empty else {}
    rows=[]
    for c in customer_df.to_dict("records"):
        cid=c["customer_id"]
        rr=ra_dict.get(cid,{})
        ss=sa_dict.get(cid,{})
        out=float(rr.get("outstanding_amount",0.0))
        lo_=str(ss.get("last_order_date",""))[:10]
        lp_=str(rr.get("lp",""))[:10]
        ytd=float(ss.get("total_business_ytd",0.0))
...
def build_proc_inv(item_df, purch_df, sales_df, stock_df) -> pd.DataFrame:
    pl_dict = purch_df.groupby("product_id")["voucher_date"].max().to_dict() if not purch_df.empty else {}
    sl_dict = sales_df.groupby("product_id")["voucher_date"].max().to_dict() if not sales_df.empty else {}
    st_dict = stock_df.groupby("product_id")["closing_qty"].sum().to_dict() if not stock_df.empty else {}
    rows=[]
    for p in item_df.to_dict("records"):
        pid=p["product_id"]
        lpd=str(pl_dict[pid])[:10] if pid in pl_dict else ""
        lsd=str(sl_dict[pid])[:10] if pid in sl_dict else ""
        cs=int(st_dict.get(pid,0))
